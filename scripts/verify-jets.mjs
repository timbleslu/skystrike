...
const dataUrl = await page.evaluate(() => {
  const SHAPESL = ['F22', 'SU57', 'J20', 'F35', 'EFT', 'TEJAS', 'RAFALE', 'FA18', 'J36', 'F47', 'J50', 'STD', 'BOMBER'];
  const W = 1500, H = 1150;
  ...
  const loaded = [];
  for (let i = 0; i < SHAPESL.length; i++) {
    const id = SHAPESL[i];
    if (!jetGLTF[id]) continue;
    ...
    clone.position.set(cx, 0, cz); sc.add(clone); loaded.push(id);
  }
  const span = cols * cell;
  const cam = new THREE.OrthographicCamera(-span / 2 - cell, span / 2 + cell, span / 2 + cell, -span / 2 - cell, 0.1, 3000);
  // rear-3/4 from above-behind-right: jets face -Z, flames at +Z toward camera
  const dir = new THREE.Vector3(0.45, 0.6, 1).normalize().multiplyScalar(600);
  cam.position.copy(dir); cam.up.set(0, 1, 0); cam.lookAt(0, 0, 0);
