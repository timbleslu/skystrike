...
const mkMat = (withMap, shared) => ({ disposed: false, userData: shared ? { shared: true } : {}, map: withMap ? { disposed: false, dispose() { this.disposed = true; } } : null, dispose() { this.disposed = true; } });
function mkGroup(children) { return { traverse(fn) { for (const c of children) fn(c); } }; }

// per-instance geometry IS disposed; shared geometry is NOT; per-instance materials
// disposed; shared materials (pooled missile assets) and textures never
const perInst = mkGeo(false), sharedG = mkGeo(true);
const mat1 = mkMat(false), mat2 = mkMat(true), sharedM = mkMat(false, true);
const group = mkGroup([
  { isMesh: true, geometry: perInst, material: mat1 },
  { isMesh: true, geometry: sharedG, material: mat2 },
  { isMesh: true, geometry: mkGeo(true), material: sharedM },
]);
disposeGroup(group);
...
assert.strictEqual(sharedM.disposed, false, 'shared material must NOT be disposed');
console.log('ok - disposeGroup skips shared geometry + shared materials, frees per-instance, spares textures');
