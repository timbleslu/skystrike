...
## Hard rules
- All new user-facing strings go through `t(key)` (static UI) or `jetText`/`techText` (JETS/TECH_TREE rows); add EN+ZH entries in `js/i18n.js`. `tests/i18n.test.js` enforces this: EN/ZH key sets must match, `{token}` placeholders must match per key, and every `JETS`/`TECH_TREE` id must have an `I18N.ZH.jet`/`.tech` override — so a missing translation fails `npm test` immediately, not on a later audit.
...
- i18n regression guard (2026-06-13): added `tests/i18n.test.js` — fails `npm test` if `I18N.EN`/`I18N.ZH` key sets diverge, if a `{token}` placeholder is dropped in translation, or if a `JETS`/`TECH_TREE` entry has no `I18N.ZH.jet`/`.tech` override. Catches missing translations the moment a new feature/jet/tech node is added (see Hard rules).
